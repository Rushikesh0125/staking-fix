...
contract StakingContract is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public rewardScale; /// Scaling factor for reward rate, typically 10**tokenDecimals.
    ...
    /// Owner-controlled switch to allow user emergency withdrawals while paused
    ...
    /**
     * @dev Initializer to set up the contract behind a proxy.
     ...
     * @param initialOwner Owner address (ideally the TimelockController).
     */
    function initialize(
        uint256 _rewardRate,
        ...
        rewardScale = scale;
        rateChanges.push(RateChange({newRate: rewardRate, changeTime: block.timestamp}));
    }

    /// UUPS authorization hook; restrict upgrades to owner (expected to be TimelockController)
    ...
    /**
     * @notice Emergency unstake of principal only while paused and owner-enabled.
     * @dev Skips staking period and reward accrual; transfers only principal.
    ...
    function emergencyUnstake(uint256 amount) external whenPaused {
        ...
        // No reward accrual and no period check in emergency flow
        userStake.balance -= amount;
        totalStaked -= amount;
        if (userStake.balance == 0) {
            userStake.lastStakeTime = 0;
    ...
    function calculateRewards(address user) public view returns (uint256) {
        ...
        for (uint256 i = startIdx; i < length; ++i) {
            ...
            unchecked {
                segmentDuration = changeTime - cursor;
            }
            accrued += (stakedAmount * currentRate * segmentDuration) / rewardScale;
        ...
        // Accrue final segment from cursor to endTime
        {
            uint256 segmentDuration;
            unchecked {
                segmentDuration = endTime - cursor;
            }
            accrued += (stakedAmount * currentRate * segmentDuration) / rewardScale;
        }
    ...
    /**
     * @notice Owner-controlled switch to enable/disable user emergency withdrawals during pause.
     * @param enabled True to enable user emergency withdrawals, false to disable
     */
    function setEmergencyWithdrawalsEnabled(bool enabled) external onlyOwner {
        emergencyWithdrawalsEnabled = enabled;
        emit EmergencyWithdrawalsToggled(enabled);
    }

    /**
     * @notice Pause the contract. Disables staking, unstaking, and claiming.
    ...
    /**
     * @notice Unpause the contract. Re-enables staking, unstaking, and claiming.
     ...
     */
    function fundRewardReserve(uint256 amount) external onlyOwner {
     ...
     */
    function withdrawFromRewardReserve(uint256 amount, address to) external onlyOwner {
     ...
     * @param to Address to withdraw tokens to
     */
    function withdrawExcessTokens(uint256 amount, address to) external onlyOwner {
        if (to == address(0)) revert ZeroAddress();
        if (amount == 0) revert AmountMustBePositive();
    ...
    function initiateEmergencyWithdraw() external onlyOwner whenPaused {
        ...
        // avoid reinitializing the schedule if it's already set and the delay hasn't elapsed
        if (availableAt != 0 && block.timestamp < availableAt) revert TimelockNotElapsed();
    ...
    function emergencyWithdraw() external onlyOwner whenPaused {
        ...
        // Withdraw the full contract balance (including user principal and reserves) to owner
